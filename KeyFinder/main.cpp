...
{
	// Prefer successFile output if set
	if(_config.successFile.length() != 0) {
		std::string wif = Address::privateKeyToWIF(info.privateKey, info.compressed);
		std::string s = info.address + " " + info.privateKey.toString(16) + " " + wif;
		util::appendToFile(_config.successFile, s);
		Logger::log(LogLevel::Info, "Found key for address '" + info.address + "'. Written to '" + _config.successFile + "'");
		if(_config.notifyCmd.length() != 0) {
			// Best-effort fire-and-forget notification
			std::string cmd = _config.notifyCmd + " \"" + s + "\"";
			system(cmd.c_str());
		}
		return;
	}
...
void usage()
{
    ...
    printf("--success FILE          Write matches to FILE as 'address hexPriv WIF'\n");
    printf("--notify-cmd CMD        Run CMD on success with quoted line appended\n");
...
int main(int argc, char **argv)
{
    ...
    parser.add("", "--success", true);
    parser.add("", "--notify-cmd", true);
	...
	for(unsigned int i = 0; i < args.size(); i++) {
		...
		try {
            ...
            } else if(optArg.equals("", "--success")) {
                _config.successFile = optArg.arg;
            } else if(optArg.equals("", "--notify-cmd")) {
                _config.notifyCmd = optArg.arg;
    ...
    if(_config.successFile.length() == 0) {
        _config.successFile = "Success.txt";
    }
	...
	if(ops.size() == 0) {
		if(_config.targetsFile.length() == 0) {
			// Default to address.txt if present
			_config.targetsFile = "address.txt";
		}
	} else {
    ...
    // If random64, constrain to 64-bit keyspace and randomize start
        ...
        _config.startKey = 1;
        _config.endKey = max64;
        _config.nextKey = 1;
        // Randomize starting point within 64-bit window for this session
        crypto::Rng rng;
        unsigned char buf[8];
        rng.get(buf, 8);
        uint64_t r =
            ((uint64_t)buf[0] << 56) | ((uint64_t)buf[1] << 48) | ((uint64_t)buf[2] << 40) |
            ((uint64_t)buf[3] << 32) | ((uint64_t)buf[4] << 24) | ((uint64_t)buf[5] << 16) |
            ((uint64_t)buf[6] << 8) | (uint64_t)buf[7];
        if(r == 0) { r = 1; }
        _config.nextKey = secp256k1::uint256(r);
        _config.startKey = _config.nextKey;
    }
	...
	if(optCompressed && optUncompressed) {
		...
		// Default for pre-2012 wallets: uncompressed
		_config.compression = PointCompressionType::UNCOMPRESSED;
	}
